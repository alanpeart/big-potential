...
<?php 
	$image="";
	$entity = $url['options']['entity'];
	if(isset($entity->field_logo['und'][0]['fid'])) {
		$image = theme('image_style',
                array(
					'style_name' => 'thumb_logo',
					'path' => $entity->field_logo['und'][0]['filename'],
					'width' => NULL,
					'height' => NULL,
					'attributes' => array(
						'class' => 'thumb_logo'
	...
	elseif(isset($entity->field_image['und'][0]['fid'])) {
		$image = theme('image_style',
                array(
					'style_name' => 'thumb_logo',
					'path' => $entity->field_image['und'][0]['filename'],
					'width' => NULL,
					'height' => NULL,
					'attributes' => array(
						'class' => 'thumb_logo'
...
<li class="search-result">
  ...
  <?php if(strlen($image) > 0): ?>
	...
	<?php endif; ?>
  <div class="search-snippet-info<?php print strlen($image) > 0 ? '' : ' noimage'; ?>">
    <?php if ($snippet) : ?>
      <?php print bp_truncate($snippet, 300); ?>
    <?php endif; ?>
  </div>
</li>
